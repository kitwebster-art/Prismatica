#!/usr/bin/env python3
"""Build a bounded watertight triangular BRep solid for Prismatica V2.
...
OUT_DIR = SAVED / "cad_bounded_triangle_brep"
...
def tri_face(a: gp_Pnt, b: gp_Pnt, c: gp_Pnt):
...
def quad_as_tris(sewing, a: gp_Pnt, b: gp_Pnt, c: gp_Pnt, d: gp_Pnt):
    sewing.Add(tri_face(a, b, c))
    sewing.Add(tri_face(a, c, d))
...
def build_solid(top, bottom):
    ...
    for i in range(n - 1):
        for j in range(n - 1):
            quad_as_tris(sewing, top[i][j], top[i][j + 1], top[i + 1][j + 1], top[i + 1][j])
            quad_as_tris(sewing, bottom[i][j], bottom[i + 1][j], bottom[i + 1][j + 1], bottom[i][j + 1])

    for j in range(n - 1):
        quad_as_tris(sewing, bottom[0][j], bottom[0][j + 1], top[0][j + 1], top[0][j])
        quad_as_tris(sewing, bottom[-1][j], top[-1][j], top[-1][j + 1], bottom[-1][j + 1])
    for i in range(n - 1):
        quad_as_tris(sewing, bottom[i][0], top[i][0], top[i + 1][0], bottom[i + 1][0])
        quad_as_tris(sewing, bottom[i][-1], bottom[i + 1][-1], top[i + 1][-1], top[i][-1])
...
def main() -> int:
    ...
    step_path = OUT_DIR / "prismatica_morph_rings_500x500_bounded_triangle_brep_solid.step"
    ...
    readme.write_text(
        "Prismatica V2 bounded watertight triangular BRep export\n"
        "============================================\n\n"
        "This file is a conservative watertight BRep solid built from bounded quad\n"
        "triangular faces sampled from the Prismatica lens heightfield. It was made after\n"
