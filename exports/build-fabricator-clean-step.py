...
Circular lenses are generated as true revolved CAD solids from the centre
...
try:
    ...
    from OCP.gp import gp_Ax1, gp_Ax2, gp_Ax3, gp_Circ, gp_Cylinder, gp_Dir, gp_Pln, gp_Pnt
except ModuleNotFoundError as exc:
    if exc.name != "OCP":
        raise
...
def footprint_description(state: dict) -> str:
    if state.get("lensType") == "round_sine_lens":
        ...
        return f"{diameter:.0f} mm diameter circle"
...
def apply_flat_edge(z: float, x: float, y: float, state: dict) -> float:
    ...
    width = float(state.get("panelWidth", WIDTH_MM))
    height = float(state.get("panelHeight", HEIGHT_MM))
    d = min(width * 0.5 - abs(x), height * 0.5 - abs(y))
    if d <= flat_edge:
        return 0.0
...
def sample_visualizer_grid(state: dict):
    ...
    enforce_flat_square_border(reference, state)
    ...
    for i in range(samples):
        ...
        values.append(row)
    enforce_flat_square_border(values, state)
    return values
...
def apply_optical_surface_cleanup(values: list[list[float]], state: dict) -> dict[str, float | bool | str]:
    ...
    for i in range(samples):
        ...
        for j in range(samples):
            x = -HALF_W + WIDTH_MM * j / (samples - 1)
            inset = min(HALF_W - abs(x), HALF_H - abs(y))
            if inset <= flat_edge:
                continue
...
def enforce_flat_square_border(values: list[list[float]], state: dict) -> None:
    ...
    B-spline fitting can reintroduce tiny relief near the square perimeter. The
    ...
    if samples < 2:
        return
    flat_edge = max(0.0, float(state.get("flatEdgeWidth", state.get("flat_edge_width", 10.0)) or 0.0))
    boundary_lock = max(flat_edge, 0.0)
    for i in range(samples):
        y = -HALF_H + HEIGHT_MM * i / (samples - 1)
        for j in range(samples):
            x = -HALF_W + WIDTH_MM * j / (samples - 1)
            d = min(HALF_W - abs(x), HALF_H - abs(y))
            if i in (0, samples - 1) or j in (0, samples - 1) or d <= boundary_lock:
                values[i][j] = 0.0
...
def visualizer_fidelity_report(state: dict, cad_grid: list[list[float]]) -> dict:
    ...
    if ref_grid is None:
        ...
        enforce_flat_square_border(ref_grid, state)
...
def circular_wire(radius: float, z: float):
    axis = gp_Ax2(gp_Pnt(0, 0, z), gp_Dir(0, 0, 1))
    edge = BRepBuilderAPI_MakeEdge(gp_Circ(axis, radius))
    if not edge.IsDone():
        raise RuntimeError("Could not build circular edge")
    wire = BRepBuilderAPI_MakeWire()
    wire.Add(edge.Edge())
    if not wire.IsDone():
        raise RuntimeError("Could not build circular wire")
    return wire.Wire()


def face_from_trimmed_surface(surface: Geom_BSplineSurface, wire):
    maker = BRepBuilderAPI_MakeFace(surface, wire, True)
    if not maker.IsDone():
        raise RuntimeError("Could not build trimmed surface face")
    return maker.Face()
...
def front_surface_from_shape(shape):
    faces = []
...
def imported_front_surface_report(shape, reference_grid: list[list[float]], state: dict) -> dict:
    ...
    front = front_surface_from_shape(shape)
...
def fabrication_qa_report(
    ...
    expected_faces = 3 if state.get("lensType") == "round_sine_lens" else 6
    topology_pass = (
...
def write_validation_pdf(
    ...
    _pdf_text(c, page_w - margin - 205, 24, f"Optical cleanup: {optical_cleanup.get('mode', 'off')} | Corner cleanup: {corner_cleanup.get('corner_flat_mm', 0):.1f} mm flat", 6.3, colors.HexColor("#777d88"))
...
def main() -> int:
    ...
    readme_path.write_text(
        ...
        "- Method: CAD height grid compared against the high-resolution visualizer field after the same fabricator corner cleanup.\n"
